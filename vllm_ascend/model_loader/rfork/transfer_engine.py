...
class RForkTransferEngineBackendWorker:
    ...
    def init_transfer_engine(self):
        ...
        # Keep compatibility with potential API variants across TE releases.
        init_ok = False
        init_errors = []
        for args in (
            (local_hostname, int(torch.npu.current_device()), 4),
            (local_hostname,),
        ):
            try:
                ret = self.rfork_transfer_engine.initialize(*args)
                if self._status_ok(ret):
                    init_ok = True
                    break
                init_errors.append(f"initialize{args} -> {ret}")
            except Exception as e:
                init_errors.append(f"initialize{args} raised: {e}")
